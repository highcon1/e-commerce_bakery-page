...
const FloatingCart = () => {
  ...
  const cartLen = carts.items.length;

  //to display the pane that overlays the screen and display the cart items
  ...
  return (
    <>
      <Card onClick={handleCart} className="hidden lg:flex fixed right-0 top-[45%] w-[90px] h-[96px] bg-[#009F7F] rounded-l-md flex-col items-center justify-center gap-3 z-[999] cursor-pointer">
          <ShoppingCart text_up={`${cartLen || 0} items`} text_below={`$${carts.total.toFixed(2) ||0.00}`} ></ShoppingCart>
      </Card>
